    ...
    if (_isNetworkAvail) {
      try {
        ...
        await post(getCartApi, body: parameter, headers: headers)
            .timeout(Duration(seconds: timeOut));
        print(getCartApi.toString());
        print('___cartapiiiiiiiiiiiiiiiii$getCartApi');
        print('___cartapiiiiiiiiiiiiiiiii$parameter');
    ...
    if (_isNetworkAvail) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          bool error = getdata["error"];
          // String msg = getdata["message"];
          print('Parameterrrrrrrrrrrrrrrrrrrrrrr${parameter}');
          print('_________________${getAddressApi}');
          if (!error) {
            var data = getdata["data"];
            addressList = (data as List)
                .map((data) => new User.fromAddress(data))
                .toList();
            // delCharge = double.parse(addressList[0].deliveryCharge.toString());

            if (addressList.length == 1) {
              print("this is working 11111");
              selectedAddress = 0;
              selAddress = addressList[0].id;
              // if (ISFLAT_DEL) {
                if (totalPrice < double.parse(addressList[0].freeAmt!)) {
                  print("working here r849834875");
                  delCharge = double.parse(addressList[0].deliveryCharge!);
                  totalamount = (double.parse(totalamount.toString()) + delCharge).toString();
                  print('___this is delivery charge${delCharge}');

                  print("___this istotal${totalamount}");
                }else{
                  delCharge = 0;
                  print("not workign herer *****");
               }
            } else {
              print("this is working 2222222");
              for (int i = 0; i < addressList.length; i++) {
                if (addressList[i].isDefault == "1") {
                  selectedAddress = i;
                  selAddress = addressList[i].id;
                  // if (!ISFLAT_DEL) {
                    if (totalPrice < double.parse(addressList[i].freeAmt!)) {
                      ...
                      totalamount =
                          (double.parse(totalamount.toString()) + delCharge)
                              .toString();
                      print('___this is delivery charge222222222222222${delCharge}');
                      print('___this is total  charge222222222222222${totalamount}');
